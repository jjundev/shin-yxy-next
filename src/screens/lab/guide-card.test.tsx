...
describe("GuideCard", () => {
  it("1단계: 진행도와 제목, 본문, 용어 없음", () => {
    ...
    const card = screen.getByRole("region", { name: "1/4 · 예측 시점과 채점 시점이 다릅니다" });
    expect(within(card).getByText(/기준 시점은 예측을 하는 날/)).toBeInTheDocument();
  ...
  it("2단계: 세 용어를 표준 라벨과 원본어로 풀고, 왜 이렇게 하나요? 가 계산 시트를 연다", async () => {
    ...
    expect(within(sheet).getByText("모은다")).toBeInTheDocument();
