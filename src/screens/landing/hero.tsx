...
export function Hero({ result }: { result: RunResult | null }) {
  ...
  return (
    <section className="relative flex min-h-dvh flex-col justify-center overflow-hidden px-4 py-24">
      ...
      <div className="mx-auto grid w-full max-w-wide items-center gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,520px)]">
        <div className="flex max-w-content flex-col gap-6">
          <p className="hero-in text-sm font-medium text-muted-foreground">{strings.appName}</p>
          <h1
            className="hero-in text-3xl leading-snug font-semibold break-keep md:text-5xl"
            style={{ "--d": 0.08 } as CSSProperties}
          >
            {strings.landing.promise}
          </h1>
          <p
            className="hero-in text-lg break-keep text-muted-foreground md:text-xl"
            style={{ "--d": 0.16 } as CSSProperties}
          >
            {strings.landing.sub}
          </p>
          <div className="hero-in flex flex-wrap items-center gap-4" style={{ "--d": 0.24 } as CSSProperties}>
